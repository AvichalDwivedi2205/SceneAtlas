...
Defaults for the next implementation turn: Clerk auth; private boards; owner/editor/viewer; copyable recipient-bound invitations; simultaneous board editing with revision conflicts; one script per board; two independent plans; California state-property permit pilot; Parallel primary; Exa disabled; desktop-first canvas. First work item: milestone 0, then shared board foundations.
