...
- The infinite canvas opens with a screenplay root. Producers choose scenes and explicitly create the plan branches they want. Each keeps its own included scenes, selections, locks, constraints, totals, and schedules. An absent scene-scope field retains legacy all-scenes behavior; an explicitly empty selection remains empty. Excluded scenes stay on the canvas and retain their saved location choices.
...
- Ready plans can generate their draft shooting orders concurrently, with separate activity and targeted retries. A cap is not a claim of budget compliance: unquoted items remain visible.
...
1. Open the **Infinite canvas** and upload a text-based screenplay PDF or plain text (up to 50 MB). The first read creates only the screenplay root.
2. Choose **Choose scenes & plans**. Read the screenplay scenes, then choose the entire screenplay or specific scenes for the shoot. Continue to add one or more plan alternatives, each with its own location budget cap or no fixed cap. Each cap covers the included scenes together; cast, crew, equipment and post-production are separate. Create only the selected branches. **Add plans** lets you choose scenes and add more alternatives later.
3. Choose **Add production details**. Review the selected scene count and location caps, then enter the shared creative brief, producer answers, shooting-day timing, scene durations and allowed windows. You can review or change scene selection here; existing caps remain unchanged and need review for the new scope. Starting research queues one shared location-research task per scene; compatible evidence is reused. No location is selected automatically.
4. Open a plan branch. Review source-backed candidates, select and lock that plan’s locations, then check the selected locations’ requirements. Switch branches to make independent choices. **All cards** and **All screenplay scenes** preserve access to the entire screenplay. After research begins, further input edits use the existing reviewed revision flow.
5. Open **Schedule**, check the plans you want to generate, then choose **Generate selected schedules** when those plans are ready. Each plan also has its own generation button. This calculates an order from confirmed choices; it does not automatically choose locations or guarantee a global optimum. Compare actual costs, missing quotes, dates, moves and blockers, then choose the plan to carry forward.
6. Edit the chosen plan’s start time. Review the previous/proposed input, **Save new start time**, **Generate updated schedule**, inspect the staged rows and preserved decisions, then **Apply updated schedule**. Other plans remain independent. Undo checks revisions before restoring inputs and dependencies.
