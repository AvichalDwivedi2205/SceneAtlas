...
2. Choose **Choose plans**. Add only the branches you want: one Budget plan, several Budget plans with different caps, a No fixed budget plan, or a mix. Set each cap and optional name, then create the selected branches. Import and scene breakdown never create plans automatically. **Add plans** creates more branches later using saved production inputs, with independent location choices.
3. Choose **Set up selected plans**. Review each cap and enter the shared creative brief, producer answers and shooting-day timing. Save the plans and break down the complete screenplay. Then choose shared scenes, enter durations and confirm allowed shooting windows. Starting the plans queues one shared location-research task per scene; compatible evidence is reused. No location is selected automatically.
