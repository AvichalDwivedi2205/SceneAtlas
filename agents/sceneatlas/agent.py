...
Return every source reference as {"url": "exact observed URL"} only. Do not repeat titles, excerpts, dates, search IDs or provider metadata: the backend attaches the original retrieved evidence. This compact reference replaces source metadata fields in the supplied canonical entity schemas. Keep descriptions and requirements concise. Do not declare that a production qualifies as a Simple or Complex Shoot from crew size or equipment alone. Keep category eligibility and category-dependent fees provisional pending authority confirmation; a public rate table is not confirmation of this production’s category.
...
Fee estimates require task.feeEstimatesAllowed=true, derived from the saved fee_estimate_policy choice. Otherwise estimated amounts stay unknown/null, even when their unit rate is published. Estimated shooting time is separate from fee-estimate permission.
...
def required_intake() -> list[dict]:
    questions = [
        ...
        intake_question("fee_estimate_policy", "How should we handle missing fees?", "Choose a suggested policy. Unquoted fees stay unknown unless you explicitly allow clearly labeled cost estimates. Estimated shooting durations or a budget cap do not approve estimated fees; an estimate does not confirm the authority's fee category or final quote.", ["research", "requirements"]),
    ]
    questions[-1]["data"]["suggestions"] = [FEE_ESTIMATES_UNKNOWN, FEE_ESTIMATES_ALLOW]
    return questions
...
def validate_requirements_target(result: dict, target: dict):
    """A requirements refresh updates one existing location, never candidates."""
    locations = result.get("locations", [])
    waiting = any("requirements" in q["data"].get("blocks", []) for q in result.get("questions", []))
    if len(locations) != 1 and not (not locations and waiting):
        raise ValueError("Requirements must refresh exactly one selected target location. Return only that location, not other candidates from the context. Copy its existing name exactly and keep its sceneIds. A question-only result is allowed only for a missing producer input blocking requirements.")
    if locations and locations[0].get("name") != target.get("name"):
        raise ValueError("Requirements returned a different location name. Copy the selected target location's name exactly; do not rename it or research another candidate. Only its requirements, costs, sources and authority are being refreshed.")
...
class SceneAtlasAgent(BaseAgent):
    ...
    async def _run_async_impl(self, ctx: InvocationContext) -> AsyncGenerator[Event, None]:
        ...
        if kind in {"research","requirements"}:
            ...
            task["feeEstimatesAllowed"] = fee_estimates_allowed(answers)
            objective, queries = research_request(target["data"], answers, kind)
